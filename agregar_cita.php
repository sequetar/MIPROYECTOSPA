...
$servicio_seleccionado = intval($_GET['servicio_id'] ?? 0);
    ...
    <form method="post" class="bg-white p-4 shadow rounded">
        ...
        <div class="mb-3">
            ...
            <select id="servicio_id" name="servicio_id" class="form-select" required>
                <option value="" disabled <?= $servicio_seleccionado === 0 ? 'selected' : '' ?>>Seleccione un servicio</option>
                <?php while ($row = mysqli_fetch_assoc($servicios)): ?>
                    <option value="<?= $row['id'] ?>"
                        <?= (isset($_POST['servicio_id']) && $_POST['servicio_id'] == $row['id']) || (!$mensaje && $servicio_seleccionado == $row['id']) ? 'selected' : '' ?>>
